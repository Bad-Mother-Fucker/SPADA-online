# Checklist — Audit output criterio

## Prima dell'audit

- [ ] Ho letto tutte le proposte in `05_criteria_outputs/Cx_output.md`
- [ ] Ho letto il criterio `03_criteria/criteria/criterion_Cx.md`
- [ ] Ho letto le schede documenti rilevanti in `04_doc_summaries/`

## Per ogni proposta

- [ ] Ho valutato la solidità dell'evidenza (forte / sufficiente / debole / assente)
- [ ] Ho verificato la coerenza con criterio e subcriterio
- [ ] Ho valutato il rischio fuori scope
- [ ] Ho verificato la compatibilità tecnica, economica, temporale
- [ ] Ho valutato la misurabilità del beneficio
- [ ] Ho assegnato uno stato (approvata / con riserva / da integrare / scartata)
- [ ] Se evidenza debole su tavola, ho richiesto riapertura della tavola originale

## Dopo l'audit

- [ ] Ho compilato la tabella audit in `Cx_output.md`
- [ ] Ho documentato le note per proposte con riserva
- [ ] Ho generato domande guida per proposte `da integrare`
- [ ] Ho documentato il motivo per proposte scartate
- [ ] Ho aggiornato `06_registers/proposal_register.md` (solo approvate)
- [ ] Ho aggiornato `06_registers/audit_summary.md`
- [ ] Ho segnalato a `context-monitor` per aggiornamento snapshot
